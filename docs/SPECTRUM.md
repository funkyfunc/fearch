...
| DuckDuckGo lite in a real browser window (the person's Chrome, or a minimised window of the installed Chrome) | Bing-syndicated | yes | none | 🟡 rung 7b: robots.txt permits `/lite/`; ToS has no automation clause; DDG's bot-check (it keys on the User-Agent — `HeadlessChrome` and token-bearing UAs get it, a real Chrome window does not; measured 2026-09-03) is put to the person to pass, never to the tool |
