...
const CODE_EXT: Record<string, string> = {
  py: "python", js: "javascript", mjs: "javascript", cjs: "javascript", ts: "typescript", mts: "typescript",
  tsx: "tsx", jsx: "jsx", rs: "rust", go: "go", java: "java", kt: "kotlin", swift: "swift", rb: "ruby",
  php: "php", c: "c", h: "c", cpp: "cpp", cc: "cpp", hpp: "cpp", cs: "csharp", sh: "bash", bash: "bash",
  zsh: "bash", yml: "yaml", yaml: "yaml", toml: "toml", json: "json", xml: "xml", html: "html", css: "css",
  scss: "scss", sql: "sql", proto: "protobuf", tf: "hcl", dockerfile: "dockerfile", lua: "lua", ex: "elixir",
  exs: "elixir", scala: "scala", hs: "haskell", ml: "ocaml", clj: "clojure", dart: "dart", r: "r", jl: "julia",
  zig: "zig", nim: "nim", vue: "vue", svelte: "svelte", ini: "ini", cfg: "ini", mk: "makefile",
  makefile: "makefile", gradle: "groovy", ps1: "powershell",
};
...
function made(url: string, body: string, source: string): Fetched {
...
async function github(url: string, u: URL, http: HttpLike): Promise<Fetched | null> {
  ...
  if (host === "raw.githubusercontent.com" || host === "gist.githubusercontent.com") {
    ...
    return made(url, body, "github-raw");
  }
  ...
  if (host === "gist.github.com" && segs.length >= 2) {
    ...
    for (const [name, f] of Object.entries((data.files ?? {}) as Record<string, J>)) {
      const [, body] = wrapFile(name, String(f.content ?? ""));
      out.push(`## ${name}\n\n${body}`);
    }
    return made(url, out.join("\n\n") + "\n", "github-gist");
  }
  ...
  if (segs.length === 2) {
    const r = await getOk(http, `https://api.github.com/repos/${owner}/${repo}/readme`, ghHeaders("application/vnd.github.raw+json"));
    if (!r) return null;
    const meta = await getOk(http, `https://api.github.com/repos/${owner}/${repo}`, ghHeaders("application/vnd.github+json"));
    let head = "";
    if (meta) {
      const m = (await meta.json()) as J;
      head =
        `# ${m.full_name}\n\n${m.description ?? ""}\n\n` +
        `Stars: ${m.stargazers_count} · Language: ${m.language} · Default branch: ${m.default_branch} · Updated: ${String(m.pushed_at).slice(0, 10)}\n\n---\n\n`;
    }
    return made(url, head + (await r.text()), "github-readme");
  }
  ...
  if (segs.length >= 4 && segs[2] === "tree") {
    ...
    for (let refLen = 1; refLen <= Math.min(rest.length, 3); refLen++) {
      ...
      const r = await getOk(http, `https://api.github.com/repos/${owner}/${repo}/contents/${path}?ref=${encodeURIComponent(ref)}`, ghHeaders("application/vnd.github+json"));
      if (!r) continue;
      const items = (await r.json()) as J[] | J;
  ...
  if (segs.length >= 3 && segs[2] === "releases") {
    ...
    if (!r) return null;
    const data = (await r.json()) as J | J[];
    const rels = Array.isArray(data) ? data : [data];
    const out = [`# ${owner}/${repo} releases${tag ? ` — ${tag}` : " (latest 10)"}`, ""];
    for (const rel of rels) {
      out.push(`## ${rel.name || rel.tag_name} (${rel.tag_name}, ${String(rel.published_at ?? rel.created_at).slice(0, 10)}${rel.prerelease ? ", pre-release" : ""})`, "", String(rel.body ?? "(no notes)").trim(), "");
    }
    return made(url, out.join("\n") + "\n", "github-releases");
  }
  ...
  if (segs.length >= 4 && (segs[2] === "issues" || segs[2] === "pull") && /^\d+$/.test(segs[3])) {
    const num = segs[3];
    const r = await getOk(http, `https://api.github.com/repos/${owner}/${repo}/issues/${num}`, ghHeaders("application/vnd.github+json"));
    if (!r) return null;
    const issue = (await r.json()) as J;
    const out = [
      `# ${issue.title} (#${num})`,
    ...
    const c = await getOk(http, `https://api.github.com/repos/${owner}/${repo}/issues/${num}/comments?per_page=15`, ghHeaders("application/vnd.github+json"));
    if (c) {
      for (const cm of (await c.json()) as J[]) {
...
async function pypi(url: string, u: URL, http: HttpLike): Promise<Fetched | null> {
  ...
  if (!r) return null;
  const info = (((await r.json()) as J).info ?? {}) as J;
  ...
  const head = [
    ...
    "Links: " + Object.entries(urls).slice(0, 6).map(([k, v]) => `${k}: ${v}`).join(" · "),
    "",
    "---",
    "",
  ];
  let desc = String(info.description ?? "");
  if (String(info.description_content_type ?? "").startsWith("text/x-rst")) desc = "(README is reStructuredText)\n\n" + desc;
  return made(url, head.join("\n") + desc + "\n", "pypi");
}
...
async function npm(url: string, u: URL, http: HttpLike): Promise<Fetched | null> {
  ...
  const head = [`# ${data.name} ${latest}`, data.description ?? "", "", `Homepage: ${data.homepage || "?"} · Repo: ${repo || "?"}`, "", "---", ""];
...
async function stackoverflow(url: string, u: URL, http: HttpLike): Promise<Fetched | null> {
  ...
  if (!q) return null;
  const items = (((await q.json()) as J).items ?? []) as J[];
  if (!items.length) return null;
  const qi = items[0];
  const who = (o: J | undefined) => (o?.display_name ? String(o.display_name) : "unknown");
  const out = [
    `# ${htmlSnippetToMarkdown(String(qi.title ?? "")).trim()}`,
    `Asked by ${who(qi.owner as J)} · score ${qi.score} · ${qi.answer_count} answers · tags: ${(qi.tags ?? []).join(", ")}`,
    "",
    htmlSnippetToMarkdown(String(qi.body ?? "")),
  ];
  const a = await getOk(http, `${base}/questions/${m[1]}/answers?site=stackoverflow&order=desc&sort=votes&filter=withbody&pagesize=4`);
  if (a) {
    const answers = (((await a.json()) as J).items ?? []) as J[];
    ...
    for (const ans of answers) {
      ...
      out.push(`\n---\n## ${tag} by ${who(ans.owner as J)} (score ${ans.score}) — ${link}\n\n${htmlSnippetToMarkdown(String(ans.body ?? ""))}`);
    }
  }
