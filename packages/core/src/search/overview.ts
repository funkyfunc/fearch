...
const CHROME_LINE_RE =
  /^(#+\s*)?(AI Overview|AI Mode (reply\b.*|replied:?)|Web Guide|Show (more|all|less)|Export|Copy|Share|Got it|Learn more|Classic Search|More|Thinking|Searching|Thinking a little longer|Generating|Save to Google (Drive|Gmail)|When you export,.*|Feedback|Report a problem|Use code with caution\.?|An AI Overview is not available.*|Can't generate.*|Deeply analyzed .*|Run expanded analysis|Help improve .*)\s*$/i;
...
/** The smallest ancestor of `el` whose text carries the disclaimer, or null when none does. */
function blockAround($: cheerio.CheerioAPI, el: Element): Element | null {
  let node: AnyNode | null = el;
  while (node && node.type === "tag") {
    const text = $(node).text();
    if (DISCLAIMER_RE.test(text)) return node as Element;
...
function labelElements($: cheerio.CheerioAPI): Element[] {
  return $("[role=heading], h1, h2, h3, div, span")
    .filter((_, e) => $(e).children().length <= 1 && LABEL_RE.test($(e).text().trim()))
    .toArray() as Element[];
}
...
export function parseGoogleOverview(html: string, query = ""): Omit<EngineSummary, "provider"> | null {
  ...
  root.find("a[href]").each((_, a) => {
    ...
    addSource(url, text.slice(0, 120));
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (DISCLAIMER_RE.test(line)) break; // the answer ends where the disclaimer begins
