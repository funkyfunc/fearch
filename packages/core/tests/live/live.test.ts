...
d("live", () => {
  beforeAll(async () => {
    state = createApp(
      settingsFromEnv({
        ...process.env,
        FEARCH_NO_CACHE: "1",
        FEARCH_AUDIT_LOG: "stderr",
        FEARCH_UA_CONTACT: "fearch-tests@example.invalid",
      }),
    );
  ...
  it("reads native markdown, a Sphinx section, a GitHub README, and a PDF", async () => {
    const md = text(
      await c.callTool({ name: "fetch", arguments: { url: "https://gofastmcp.com/servers/tools", max_chars: 2000 } }),
    );
    ...
    const gh = text(
      await c.callTool({ name: "fetch", arguments: { url: "https://github.com/deedy5/ddgs", max_chars: 1000 } }),
    );
    expect(gh).toContain("source: github-readme");
    const pdf = text(
      await c.callTool({ name: "fetch", arguments: { url: "https://arxiv.org/pdf/2511.16397", max_chars: 800 } }),
    );
