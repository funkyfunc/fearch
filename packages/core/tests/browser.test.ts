...
function makeFetcher(opts: { renderer?: BrowserRenderer | { render: (u: string) => Promise<{ html: string; finalUrl: string; status: number; salvaged: boolean }>; enabled: () => boolean } ; env?: Record<string, string> }) {
  const settings = settingsFromEnv({ FEARCH_NO_CACHE: "1", FEARCH_AUDIT_LOG: "off", FEARCH_LOG_LEVEL: "error", FEARCH_ALLOW_PRIVATE: "1", FEARCH_PER_HOST_DELAY_MS: "1", ...(opts.env ?? {}) });
...
describe("browser ladder (fake renderer)", () => {
  ...
  it("uses the renderer for JS shells and 403s, exactly once, and stops when the browser is refused too", async () => {
    ...
    const fake = {
      ...
      async render(u: string) {
        ...
        if (u.endsWith("/shell")) return { html: JS_PAGE.replace('<div id="app"></div>', "<main><h1>Rendered Guide</h1><p>" + "Rendered. ".repeat(40) + "</p><pre><code>npm install x</code></pre></main>"), finalUrl: u, status: 200, salvaged: false };
  ...
  it("never renders when robots disallows, and reports when the browser is unavailable", async () => {
    const calls: string[] = [];
    const fake = { enabled: () => true, async render(u: string) { calls.push(u); throw new BrowserUnavailable("no chromium"); } };
    const { fetcher } = makeFetcher({ renderer: fake });
    try {
    ...
    const settings = settingsFromEnv({ FEARCH_NO_CACHE: "1", FEARCH_AUDIT_LOG: "off", FEARCH_ALLOW_PRIVATE: "1", FEARCH_PER_HOST_DELAY_MS: "1" });
    const audit = new Audit(settings);
    const cache = new Cache(null);
    const robots = new RobotsChecker(cache, async () => ({ status: 200, body: "User-agent: *\nDisallow: /\n" }));
    const f = new Fetcher(settings, cache, new Transport(settings, audit), robots, new Politeness(1, { count: 100, windowMs: 60_000 }), audit, fake as unknown as BrowserRenderer);
  ...
  it("is skipped entirely when FEARCH_BROWSER=off", async () => {
    const calls: string[] = [];
    const fake = { enabled: () => false, async render(u: string) { calls.push(u); return { html: REAL, finalUrl: u, status: 200, salvaged: false }; } };
    const { fetcher } = makeFetcher({ renderer: fake, env: { FEARCH_BROWSER: "off" } });
    await expect(fetcher.fetch(`${base}/browsers-only`)).rejects.toMatchObject({ diagnosis: { kind: "blocked_or_waf" } });
...
describe("browser tier (real Chromium)", () => {
  ...
  beforeAll(async () => {
    server = createServer((req, res) => {
      if (req.url === "/shell") return res.writeHead(200, { "content-type": "text/html" }).end(JS_PAGE);
      if (req.url === "/ua") return res.writeHead(200, { "content-type": "text/html" }).end(`<html><body><main><p>UA: ${req.headers["user-agent"]}</p><p>From: ${req.headers["from"]}</p><p>X-Agent: ${req.headers["x-agent"]}</p><p>${"x ".repeat(150)}</p></main></body></html>`);
      res.writeHead(404).end();
    });
    await new Promise<void>((r) => server.listen(0, "127.0.0.1", r));
    base = `http://127.0.0.1:${(server.address() as { port: number }).port}`;
    const settings = settingsFromEnv({ FEARCH_NO_CACHE: "1", FEARCH_AUDIT_LOG: "off", FEARCH_LOG_LEVEL: "error", FEARCH_ALLOW_PRIVATE: "1" });
  ...
  it("renders a JS-only page and identifies itself honestly", async () => {
    ...
    const none = new BrowserRenderer(settingsFromEnv({ FEARCH_NO_CACHE: "1", FEARCH_AUDIT_LOG: "off", FEARCH_LOG_LEVEL: "error", FEARCH_ALLOW_PRIVATE: "1", FEARCH_BROWSER_IDENTITY: "none" }), new Audit({ auditLog: "off", logLevel: "error" }));
    try {
  ...
  it("blocks private subresources and non-http schemes at the request gate", async () => {
    ...
    const strict = new BrowserRenderer(settingsFromEnv({ FEARCH_NO_CACHE: "1", FEARCH_AUDIT_LOG: "off", FEARCH_LOG_LEVEL: "error" }), new Audit({ auditLog: "off", logLevel: "error" }));
    try {
