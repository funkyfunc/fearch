...
import { ArxivProvider, GitHubProvider, HackerNewsProvider, MarginaliaProvider, MdnProvider, OpenAlexProvider, StackExchangeProvider } from "../src/search/federation.js";
import { canonicalize, dedupe, filterDomains, SearchError, type SearchProvider, type SearchResult } from "../src/search/provider.js";
  ...
  return async (url, init) => {
    ...
    for (const [prefix, payload] of Object.entries(routes)) {
      if (key.startsWith(prefix)) {
        return { status: 200, headers: { "content-type": "application/json" }, text: async () => JSON.stringify(payload), json: async () => payload };
      }
    }
...
describe("provider helpers", () => {
  it("canonicalizes and dedupes", () => {
    ...
    expect(dedupe([r("https://example.com/p?utm_source=x"), r("https://www.example.com/p/"), r("https://example.com/q")]).length).toBe(2);
  });
  it("filters domains", () => {
...
describe("exa hosted parsing", () => {
  it("parses JSON and text shapes", () => {
    const json = parseExaToolText(JSON.stringify({ results: [{ title: "A", url: "https://a.test/", highlights: ["h1", "h2"] }] }), "exa-hosted");
    expect(json).toEqual([{ title: "A", url: "https://a.test/", snippet: "h1 h2", provider: "exa-hosted" }]);
    const text = parseExaToolText("Title: B\nURL: https://b.test/x\nPublished: 2026\nSome summary here.\n\nTitle: C\nURL: https://c.test/\nMore.", "exa-hosted");
...
describe("federation providers", () => {
  it("github merges repos and issues for code searches, repos only otherwise", async () => {
    const http = fakeHttp({
      "https://api.github.com/search/repositories": { items: [{ full_name: "o/r", stargazers_count: 5, html_url: "https://github.com/o/r", description: "d", language: "Go", pushed_at: "2026-01-01T12:00:00Z" }] },
      "https://api.github.com/search/issues": { items: [{ title: "Bug", state: "open", html_url: "https://github.com/o/r/issues/1", body: "b" }] },
    });
  ...
  it("stackexchange and mdn", async () => {
    const http = fakeHttp({
      "https://api.stackexchange.com/2.3/search/excerpts": { items: [{ item_type: "question", title: "How &amp; why?", score: 3, is_answered: true, question_id: 42, excerpt: "<b>x</b> y" }] },
      "https://developer.mozilla.org/api/v1/search": { documents: [{ title: "Array.prototype.map()", mdn_url: "/en-US/docs/x", summary: "s" }] },
    });
    const so = await new StackExchangeProvider(http).search({ query: "q", maxResults: 5 });
    expect(so[0]).toMatchObject({ title: "How & why? (score 3, answered)", url: "https://stackoverflow.com/questions/42", snippet: "x y" });
...
describe("new keyless providers", () => {
  it("hacker news, openalex, marginalia parse their shapes", async () => {
    const http = fakeHttp({
      "https://hn.algolia.com/api/v1/search": { hits: [{ title: "Show HN: Thing", url: "https://thing.test/", objectID: "123", points: 42, num_comments: 7, created_at: "2026-05-01T10:00:00.000Z" }, { title: "Ask HN", url: null, objectID: "124", points: 3 }] },
      "https://api.openalex.org/works": { results: [{ display_name: "Paper A", id: "https://openalex.org/W1", doi: "https://doi.org/10.1/x", primary_location: { landing_page_url: "https://journal.test/a", source: { display_name: "JOSS" } }, cited_by_count: 12, publication_date: "2025-03-04" }] },
      "https://api2.marginalia-search.com/search": { results: [{ url: "https://blog.test/post", title: "A post", description: "d" }], license: "CC-BY-NC-SA" },
    });
    const hn = await new HackerNewsProvider(http).search({ query: "q", maxResults: 5 });
    expect(hn[0]).toMatchObject({ title: "Show HN: Thing (42 points, 7 comments)", url: "https://thing.test/", date: "2026-05-01" });
  ...
  it("arxiv parses Atom", async () => {
    ...
    expect(out[0]).toMatchObject({ title: "AICC: Parse HTML Finer", url: "https://arxiv.org/abs/2511.16397v2", snippet: "An abstract.", date: "2025-11-20" });
  });
});

describe("search modes", () => {
  it("first-party mode has no web providers; off mode refuses with guidance", async () => {
    const fp = new SearchRegistry(settingsFromEnv({ FEARCH_SEARCH_MODE: "first-party" } as NodeJS.ProcessEnv), new Cache(null), new Audit({ auditLog: "off", logLevel: "error" }), fakeHttp({}));
    expect(fp.web.length).toBe(0);
    expect(fp.federation.length).toBeGreaterThan(5);
    expect(fp.describe()).toContain("mode=first-party");
    const off = new SearchRegistry(settingsFromEnv({ FEARCH_SEARCH_MODE: "off" } as NodeJS.ProcessEnv), new Cache(null), new Audit({ auditLog: "off", logLevel: "error" }), fakeHttp({}));
    await expect(off.search({ query: "q", maxResults: 3 })).rejects.toThrow(/disabled/);
  });
});

describe("registry", () => {
  const stub = (name: string, results: SearchResult[] = [], error?: string, kinds: SearchProvider["kinds"] = ["web"]): SearchProvider & { calls: number } => ({
    name,
  ...
  function registry(web: SearchProvider[], federation: SearchProvider[]) {
    ...
    const reg = new SearchRegistry(settings, new Cache(null), new Audit({ auditLog: "off", logLevel: "error" }), fakeHttp({}));
  ...
  it("interleaves peer providers for kind searches and cools down a rate-limited provider", async () => {
    const mdn = stub("mdn", [r("https://mdn.test/1"), r("https://mdn.test/2"), r("https://mdn.test/3")], undefined, ["docs"]);
    ...
    const limited = stub("exa-hosted", [], "exa-hosted: rate-limited (keyless casual-use tier); waiting a few minutes lifts it.");
    const gh = stub("github", [r("https://github.com/o/r")], undefined, ["code"]);
  ...
  it("errors when nothing works", async () => {
    await expect(registry([stub("a", [], "a down")], [stub("stackexchange", [], "so down", ["qa"])]).search({ query: "q", maxResults: 2 })).rejects.toThrow(/a down; so down/);
  });

  it("renders with disclosure", () => {
    const a = stub("a", [r("https://x.com/1", "One")]);
    const text = renderResults("q", { results: [{ ...r("https://x.com/1", "One"), excerpt: "e1\ne2", date: "2026-04-23" }], providers: [a], fromCache: false, fellBackToFederation: false, notes: [] });
