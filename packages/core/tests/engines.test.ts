...
import { ENGINE_SPECS, EngineProvider, engineProviders, isBotCheck, parseBing, parseGoogle, parseLite, unwrapBing } from "../src/search/engines.js";
...
function fakeBrowser(render: (url: string) => Promise<{ html: string; status: number }>, headed = false) {
  return { enabled: () => true, headed, render: async (u: string) => ({ ...(await render(u)), finalUrl: u, salvaged: false, usedSession: false, handedOff: false }) } as unknown as BrowserRenderer;
}
function provider(name: string, render: (url: string) => Promise<{ html: string; status: number }>, env: Record<string, string> = {}, robotsBody = "User-agent: *\nDisallow: /search\nDisallow: /html/\nAllow: /lite/\n") {
  const robots = new RobotsChecker(new Cache(null), async () => ({ status: 200, body: robotsBody }), false, settings(env).robotsPolicy);
  return new EngineProvider(ENGINE_SPECS[name], settings(env), fakeBrowser(render), robots, new Politeness(1, { count: 100, windowMs: 60_000 }), 1);
}

describe("engine parsers", () => {
  it("DDG lite: unwraps redirects, drops DDG's own links", () => {
    const r = parseLite(LITE, "duckduckgo");
    expect(r.map((x) => x.url)).toEqual(["https://github.com/mixmark-io/turndown-plugin-gfm", "https://www.npmjs.com/package/turndown-plugin-gfm"]);
    expect(r[0].snippet).toContain("GitHub Flavored");
    expect(isBotCheck(202, "")).toBe(true);
    expect(isBotCheck(200, BOTCHECK)).toBe(true);
  });
  it("Bing: decodes /ck/a?u=a1<base64url> links, skips Bing's own", () => {
    expect(unwrapBing(`https://www.bing.com/ck/a?u=a1${b64("https://example.com/x?y=1")}`)).toBe("https://example.com/x?y=1");
    const r = parseBing(BING, "bing");
    expect(r.map((x) => x.url)).toEqual(["https://www.npmjs.com/package/@joplin/turndown-plugin-gfm", "https://github.com/trutohq/turndown-plugin-gfm"]);
    expect(r[0].snippet).toContain("GitHub Flavored");
    // a normal results page mentioning "challenge" in a script is not a challenge
    expect(ENGINE_SPECS.bing.isChallenge(200, BING.replace("</body>", "<script>var challenge=1</script></body>"))).toBe(false);
  ...
  it("Google: a:has(h3) results, /url?q= unwrapped, Google's own links skipped, challenge recognised", () => {
    ...
    expect(g.map((x) => x.url)).toEqual(["https://playwright.dev/docs/auth", "https://www.browserstack.com/guide/playwright-cookies", "https://example.org/a/b"]);
    ...
    expect(ENGINE_SPECS.google.isChallenge(200, GOOGLE_SORRY, "https://www.google.com/sorry/index?continue=x")).toBe(true);
    ...
    expect(ENGINE_SPECS.google.isChallenge(200, GOOGLE + "<script>var u='/sorry/index';var r='recaptcha'</script>", "https://www.google.com/search?q=x")).toBe(false);
  });
});
...
describe("engine eligibility — the dials play together", () => {
  it("DuckDuckGo is eligible by default; Bing/Google only when robots.txt is off and they are listed", () => {
    const mk = (env: Record<string, string>) => engineProviders(settings(env), fakeBrowser(async () => ({ html: "", status: 200 })), new RobotsChecker(new Cache(null), async () => ({ status: 404, body: "" })), new Politeness(1, { count: 1, windowMs: 1 }));
    ...
    expect(names(mk({ FEARCH_ENGINES: "google,duckduckgo", FEARCH_ROBOTS_POLICY: "off" }))).toEqual(["google", "duckduckgo"]);
  ...
  it("the registry orders eligible engines in FEARCH_ENGINES order and reports the rest in describe()", () => {
    ...
    const engines = engineProviders(s, fakeBrowser(async () => ({ html: "", status: 200 })), new RobotsChecker(new Cache(null), async () => ({ status: 404, body: "" }), false, "off"), new Politeness(1, { count: 1, windowMs: 1 }));
    const reg = new SearchRegistry(s, new Cache(null), new Audit(s), { get: async () => { throw new Error("no"); } } as never, engines);
    expect(reg.web.map((p) => p.name)).toEqual(["bing", "duckduckgo", "google"]);
    const s2 = settings({ FEARCH_ENGINES: "google,duckduckgo" });
    const reg2 = new SearchRegistry(s2, new Cache(null), new Audit(s2), { get: async () => { throw new Error("no"); } } as never, engineProviders(s2, fakeBrowser(async () => ({ html: "", status: 200 })), new RobotsChecker(new Cache(null), async () => ({ status: 404, body: "" })), new Politeness(1, { count: 1, windowMs: 1 })));
  ...
  it("tells the model inline when a listed engine was skipped by the robots dial", async () => {
    ...
    const robots = new RobotsChecker(new Cache(null), async () => ({ status: 200, body: "User-agent: *\nAllow: /lite/\nDisallow: /search\n" }));
    const engines = engineProviders(s, fakeBrowser(async () => ({ html: LITE, status: 200 })), robots, new Politeness(1, { count: 100, windowMs: 60_000 }));
    const reg = new SearchRegistry(s, new Cache(null), new Audit(s), { get: async () => { throw new Error("no"); } } as never, engines);
    const out = await reg.search({ query: "turndown", maxResults: 3 });
    expect(out.providers.map((p) => p.name)).toEqual(["duckduckgo"]);
    expect(out.notes.join("\n")).toMatch(/google: listed in FEARCH_ENGINES but not used — www\.google\.com disallows .*--robots off/);
  });

  it("searches through the browser and honours the live robots.txt under the current policy", async () => {
    const seen: string[] = [];
    const p = provider("duckduckgo", async (u) => { seen.push(u); return { html: LITE, status: 200 }; });
    ...
    await expect(provider("google", async () => ({ html: GOOGLE, status: 200 })).search({ query: "x", maxResults: 5 })).rejects.toThrow(/robots\.txt disallows/);
    // …and under robots=off it is opened and parsed.
    const g = await provider("google", async () => ({ html: GOOGLE, status: 200 }), { FEARCH_ROBOTS_POLICY: "off" }).search({ query: "x", maxResults: 5 });
  ...
  it("treats a bot-check page as a final 'no' (rate-limited, no retry) and says how a person could pass it", async () => {
    let calls = 0;
    const p = provider("duckduckgo", async () => { calls++; return { html: BOTCHECK, status: 202 }; });
...
describe("human handoff loop", () => {
  it("waits until the page stops being a challenge, or gives up at the deadline", async () => {
    ...
    const r = await waitForHuman(async () => ({ html: pages[Math.min(n++, 2)], status: 200, url: "https://x.test/" }), (h) => /captcha/.test(h), 10_000, 1, async () => {});
    expect(r.passed).toBe(true);
    expect(r.html).toBe("<main>results</main>");
    expect(n).toBe(3);
    const gaveUp = await waitForHuman(async () => ({ html: "<b>captcha</b>", status: 200, url: "https://x.test/" }), (h) => /captcha/.test(h), 0, 1, async () => {});
...
describe("server flags", () => {
  it("maps flags onto settings, wins over env, derives engines, and passes the rest through", () => {
    ...
    expect([a.settings.robotsPolicy, a.settings.browser, a.settings.handoff, a.settings.engines]).toEqual(["off", "headed", true, ["google", "duckduckgo"]]);
    expect(a.rest).toEqual(["search", "some query", "--n", "3"]);
    const b = settingsFromArgs(["--robots=strict", "--browser=off", "--engines", "bing,duckduckgo", "doctor"], { ...base, FEARCH_ROBOTS_POLICY: "off" });
    expect([b.settings.robotsPolicy, b.settings.browser, b.settings.engines, b.rest]).toEqual(["strict", "off", ["bing", "duckduckgo"], ["doctor"]]);
    ...
    expect(settingsFromArgs(["--robots", "off", "--browser", "headed", "--handoff"], base).settings.engines).toEqual(["google", "duckduckgo"]);
...
describe("config dials", () => {
  it("parses browser/identity/handoff/session/engines/robots and keeps headed-only dials off in headless mode", () => {
    const d = settings();
    expect([d.browser, d.browserIdentity, d.handoff, d.browserSession, d.engines, d.robotsPolicy, d.ignoreRobots]).toEqual(["headless", "header", false, false, ["duckduckgo"], "default", false]);
    const h = settings({ FEARCH_BROWSER: "headed", FEARCH_HANDOFF: "1", FEARCH_BROWSER_SESSION: "on", FEARCH_BROWSER_IDENTITY: "none", FEARCH_ENGINES: "Google, bing", FEARCH_ROBOTS_POLICY: "off" });
    expect([h.browser, h.browserIdentity, h.handoff, h.browserSession, h.engines, h.robotsPolicy, h.ignoreRobots]).toEqual(["headed", "none", true, true, ["google", "bing"], "off", true]);
    ...
    expect(settingsFromArgs(["--exa"], { FEARCH_NO_CACHE: "1", FEARCH_AUDIT_LOG: "off", FEARCH_LOG_LEVEL: "error" }).settings.exaHostedUrl).toBe("https://mcp.exa.ai/mcp");
  });
});
