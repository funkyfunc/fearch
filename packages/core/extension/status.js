chrome.runtime.sendMessage({ type: "status" }, (s) => {
  ...
  const live = Object.entries(s.servers || {})
    .filter(([, v]) => v.connected && Date.now() - v.at < 40000)
    .map(([p]) => p);
  el.innerHTML =
    (live.length
      ? `<div class="ok">✔ connected to fearch on port ${live.join(", ")}</div>`
      : `<div class="no">✘ no fearch server found — start fearch (or run <code>fearch extension status</code>)</div>`) +
