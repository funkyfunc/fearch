...
That prints the extension's folder (`packages/core/extension` in a clone or a normal install; only an
`npx` run copies it out of npm's cache to `~/fearch-extension`), puts the path on your clipboard, opens
`chrome://extensions`, and waits for the extension to connect.
